<%@ WebHandler Language="C#" Class="dataPersonalCalendar" %>

using System;
using System.Web;

public class dataPersonalCalendar : IHttpHandler {

    public void ProcessRequest(HttpContext context)
    {
        string id = context.Request.QueryString["id"];
        string[] aColor = { "#0199Cb", "#ff6600", "#339933", "#CCCCCC", "#66cc00" };
        string sData = "";
        ...
        if (!string.IsNullOrEmpty(id))
        {
            sql = string.Format(@"SELECT ROW_NUMBER() OVER(ORDER BY PERIOD_DATE_EN, PERIOD_START, PERIOD_STOP) SEQ, *
                                ...
                                (
                                    ...
                                    WHERE TMP.DEL_FLAG = 'N' AND PERSONAL_ID = {0}
                                    UNION
...
'ส่วนตัว'  as 'STATUS',
                                    ...
                                    WHERE TPC.PERSONAL_ID = {0}
                                ) AS R1
                                ORDER BY PERIOD_DATE_EN, PERIOD_START, PERIOD_STOP", id);
            ...
            for (int i = 0; i < dt.Rows.Count; ++i)
            {
                ...
                sBgColor = aColor[int.Parse(dt.Rows[i]["TYPE"].ToString())];
                ...
                sData += template.Replace("%ID%", sId).Replace("%STATUS%", sStatus).Replace("%START%", sStart).Replace("%END%", sEnd).Replace("%TEXT%", sText).Replace("%PERSONALID%", sPersonalId).Replace("%BGCOLOR%", sBgColor).Replace("%COLOR%", sTextColor);
            }
        }
